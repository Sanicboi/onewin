...
AppDataSource.initialize().then(async () => {
    ...
    const bot = new TelegramBot('7476086004:AAEcIyTsXyE2N38Pd4AqAPIokyFYcAry7Mk', {
        polling: true
    });
    ...
    app.get('/redirect/:id', async (req, res) => {
        ...
        if (user) {
            ...
            await bot.sendPhoto(+user.id, 'https://ibb.co/NpKQdd7', {
    ...
    bot.onText(/\/start/, async (msg) => {
        ...
        bot.sendPhoto(msg.from.id, 'https://ibb.co/jksGqWR', {
    ...
    bot.onText(/./, async (msg) => {
        if (!msg.text.startsWith('/')) {
            ...
            if (!signup) {
                await bot.sendMessage(msg.from.id, '❗️Неккоректный ввод ID🤨\nПовторите ввод id используя только цифры нового игрового аккаунта!');
                return;
            }
    ...
    bot.on('callback_query', async (q) => {
        if (q.data === 'free') {
            bot.sendPhoto(q.from.id, 'https://ibb.co/2sxgRs6', {
            ...
            if (!user.deposited) {
                await bot.sendPhoto(q.from.id, 'https://ibb.co/NpKQdd7', {
        ...
        bot.sendPhoto(q.from.id, 'https://ibb.co/NpKQdd7', {
            caption: 'ℹ️ Выбери тариф, для доступа в мою приватную группу с сигналами.\n\n📲Повторюсь, я готов дать тебе бесплатный доступ на 3 дня, чтобы ты убедился в том, что этот бот работает и на нем можно зарабатывать в десятки раз больше, чем стоимость подписки!',
            reply_markup: {
                inline_keyboard: [
                    [
