...
AppDataSource.initialize().then(async () => {
    ...
    const bot = new TelegramBot('7264713965:AAGp-mmR53IJnxOCFKZH2defd0RNMOx-kjQ', {
        polling: true
    });
    ...
    bot.onText(/\/start/, async (msg) => {
        ...
        bot.sendPhoto(msg.from.id, 'https://ibb.co/qpgg9TD', {
    ...
    bot.on('callback_query', async (q) => {
        if (q.data === 'free') {
            bot.sendPhoto(q.from.id, 'https://ibb.co/B2YgnNp', {
            ...
            if (!user.deposited) {
                await bot.sendPhoto(q.from.id, 'https://ibb.co/Fm9P5S1', {
