...
AppDataSource.initialize().then(async () => {
    ...
    const bot = new TelegramBot('7457782519:AAETzLPDFJbbf8qsCSlwmh10O3AmhuysV4k', {
        polling: true
    });
    ...
    bot.onText(/\/start/, async (msg) => {
        ...
        bot.sendPhoto(msg.from.id, 'https://ibb.co/2KjfYps', {
    ...
    bot.on('callback_query', async (q) => {
        if (q.data === 'free') {
            bot.sendPhoto(q.from.id, 'https://ibb.co/v3H1zM7', {
            ...
            if (!user.deposited) {
                await bot.sendPhoto(q.from.id, 'https://ibb.co/LdYGvWp', {
        ...
        bot.sendPhoto(q.from.id, 'https://ibb.co/Fm9P5S1', {
            caption: 'ℹ️ Выбери тариф, для доступа в мою приватную группу с сигналами.\n\n📲Повторюсь, я готов дать тебе бесплатный доступ на 3 дня, чтобы ты убедился в том, что этот бот работает и на нем можно зарабатывать в десятки раз больше, чем стоимость подписки!',
            reply_markup: {
                inline_keyboard: [
                    [
